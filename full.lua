...
local SCRIPT_URL = "https://raw.soteria.rip/67da390516522d6fde6fc178cd71aecb" -- 主脚本（不再加载 Key 验证系统）
...
getgenv().NNVN_OctoSpeed = 1 -- 章鱼磨血速度(倍速, 1=原版 10=最快)
...
getgenv().NNVN_SpinnerSpeed       = 500   -- 旋转速度(度/秒)
...
getgenv().NNVN_LockStamina      = false -- 锁定体力(满值)
getgenv().NNVN_BossGodMode       = false -- 保留变量兼容(逻辑已改为精准锁白线)
...
-- ====================================================
-- 0.4 自我持久化：尝试保存自身源码，换服后可恢复汉化版（不弹Key）
-- ====================================================
task.spawn(function()
    pcall(function()
        local info = debug.getinfo(1, "S")
        local srcName = info and info.source or ""
        if srcName:sub(1, 1) == "@" then
            local ok, d = pcall(readfile, srcName:sub(2))
            if ok and d and #d > 100000 then
                pcall(writefile, "xuetianzhongxin.lua", d)
            end
        end
    end)
end)
...
-- ====================================================
-- 启动：无动画，公告直接展示（确认后进入主脚本）
-- ====================================================
getgenv().NNVN_BootDone = true -- 跳过动画等待，公告立即显示
-- 1. WindUI（多源加载 + 重试，避免网络问题导致白屏）
-- ====================================================


-- ============================================================
-- WindUI 二级加载：缓存(离线秒开) → 远程多源(并行抢跑，挂梯子即用)
-- 远程成功一次后自动缓存，之后不挂梯子也能启动
-- ============================================================
local WindUI
local winduiOk = false
local WINDUI_CACHE_FILE = "雪天中心_WindUI.lua"
local WINDUI_SOURCES = {
    "https://fastly.jsdelivr.net/gh/Footagesus/WindUI@main/dist/main.lua",
    "https://gcore.jsdelivr.net/gh/Footagesus/WindUI@main/dist/main.lua",
    ...
    "https://mirror.ghproxy.com/https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua",
    "https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua",
}
local lastGoodSrc = nil
-- ① 本地缓存（离线秒启动）
pcall(function()
    if winduiOk then return end
    if readfile then
        local cached = readfile(WINDUI_CACHE_FILE)
        if cached and #cached > 1000 then
            local f = loadstring(cached)
            if f then
                local lib = f()
                if lib then WindUI = lib; winduiOk = true end
            end
        end
    end
end)
...
local function tryRemote()
    ...
    while not winduiOk and os.clock() < deadline do
        task.wait(0.3)
    end
end
for attempt = 1, 3 do
...
-- 远程成功后写缓存（下次离线秒开）
if winduiOk and lastGoodSrc and writefile then
    pcall(function() writefile(WINDUI_CACHE_FILE, lastGoodSrc) end)
end
-- ③ 加载失败：清晰提示（挂梯子/换网络后重试）
if not WindUI then
    warn("[雪天中心] WindUI 加载失败")
    getgenv().NNVN_BootDone = true
    pcall(function()
        local esg = Instance.new("ScreenGui")
        esg.Name = "NNVN_Err"
        esg.ResetOnSpawn = false
        esg.DisplayOrder = 300
        esg.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
        local ebg = Instance.new("Frame")
        ebg.Size = UDim2.fromScale(1, 1)
        ebg.BackgroundColor3 = Color3.fromRGB(10, 10, 24)
        ebg.BorderSizePixel = 0
        ebg.Parent = esg
        local etxt = Instance.new("TextLabel")
        etxt.AnchorPoint = Vector2.new(0.5, 0.5)
        etxt.Position = UDim2.fromScale(0.5, 0.45)
        etxt.Size = UDim2.fromOffset(440, 140)
        etxt.BackgroundTransparency = 1
        etxt.Font = Enum.Font.NotoSans; etxt.FontWeight = Enum.FontWeight.Bold
        etxt.TextSize = 16
        etxt.TextWrapped = true
        etxt.Text = "⚠️ UI 界面库加载失败\n\n7 个源均已超时。请挂梯子或切换网络后重新执行脚本"
        etxt.TextColor3 = Color3.fromRGB(255, 150, 150)
        etxt.Parent = ebg
        local ebtn = Instance.new("TextButton")
        ebtn.AnchorPoint = Vector2.new(0.5, 0)
        ebtn.Position = UDim2.fromScale(0.5, 0.72)
        ebtn.Size = UDim2.fromOffset(180, 38)
        ebtn.BackgroundColor3 = Color3.fromRGB(120, 40, 60)
        ebtn.Font = Enum.Font.NotoSans; ebtn.FontWeight = Enum.FontWeight.Bold
        ebtn.TextSize = 15
        ebtn.Text = "知道了"
        ebtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        local euc = Instance.new("UICorner"); euc.CornerRadius = UDim.new(0, 8); euc.Parent = ebtn
        ebtn.Parent = ebg
        local function edestroy() pcall(function() esg:Destroy() end) end
        ebtn.Activated:Connect(edestroy)
        ebtn.MouseButton1Click:Connect(edestroy)
        ebtn.TouchTap:Connect(edestroy)
    end)
    return
end
...
-- 假控件兜底：真实控件创建失败时返回，保证后续代码不中断
local function fakeControl()
    local f = { __NNVN_Fake = true }
    function f:SetValue(v) return v end
    function f:SetDesc(v) return v end
    function f:SetTitle(v) return v end
    function f:SetContent(v) return v end
    function f:AddToggle() return f end
    function f:AddButton() return f end
    function f:AddInput() return f end
    function f:AddSlider() return f end
    function f:AddDropdown() return f end
    function f:AddParagraph() return f end
    function f:AddSection() return f end
    return f
end
...
local function patchContainer(container)
    ...
    if not container.AddSection and container.Section then
        function container:AddSection(title)
            local ok, section = pcall(function()
                return self:Section({ Title = title, Opened = true, TextSize = 16, DescTextSize = 13 })
            end)
            if not ok or not section then return fakeControl() end
    ...
    if not container.AddToggle and container.Toggle then
        function container:AddToggle(flag, config)
            ...
            if not ok or not ctrl then return fakeControl() end
            return ctrl
        end
    end
    ...
    if not container.AddDropdown and container.Dropdown then
        function container:AddDropdown(flag, config)
            ...
            local okD, dropdown = pcall(function() return attachSetValue(self:Dropdown(config)) end)
            if not okD or not dropdown then return fakeControl() end
    ...
    if not container.AddButton and container.Button then
        function container:AddButton(config)
            local ok, btn = pcall(function() return self:Button(config or {}) end)
            if not ok or not btn then return fakeControl() end
            return btn
        end
    end
...
-- 通用智能点击：手机端用触摸模拟（不切桌面模式，移动键/钓鱼键保留），PC 用鼠标
-- 一律先 Activate 直接触发按钮逻辑（保证 100% 生效）
local function SmartClick(guiObject)
    if not guiObject then return false end
    local ok = pcall(function()
        if guiObject.Activate then pcall(function() guiObject:Activate() end) end
        ...
        if isTouchDevice then
            -- 手机：触摸模拟，不切换输入模式（摇杆/钓鱼键保留）
            VirtualInput:SendTouchEvent(x, y, 0, Enum.UserInputState.Begin, game)
            VirtualInput:SendTouchEvent(x, y, 0, Enum.UserInputState.End, game)
        else
...
-- SmartClickAt：按屏幕坐标点击（Boss 战绿框中心，不依赖按钮对象）
local function SmartClickAt(x, y)
    pcall(function()
        if isTouchDevice then
            VirtualInput:SendTouchEvent(x, y, 0, Enum.UserInputState.Begin, game)
            VirtualInput:SendTouchEvent(x, y, 0, Enum.UserInputState.End, game)
...
local function TeleportToCFrame(cf)
    ...
    local target = cf + Vector3.new(0,5,0)
    pcall(function() root.CFrame = target; root.Velocity = Vector3.new(0,0,0) end)
    -- 防回弹：连续 8 帧强制刷新目标位置（服务器回滚时立即拉回）
    task.spawn(function()
        for _ = 1, 8 do
            task.wait(0.03)
            pcall(function()
...
-- ====================================================
-- UI 灵动动画：按钮按压缩放 + Tab/内容切换丝滑淡入
-- ====================================================
task.spawn(function()
    task.wait(3.5) -- 等 UI 完全构建
    pcall(function()
        -- 灵动动画 v2（防重入：动画中连点不叠加、不卡死）
        local function injectAnim(root)
            for _, v in pairs(root:GetDescendants()) do
                if v:IsA("TextButton") and not v:GetAttribute("NNVN_Anim") then
                    v:SetAttribute("NNVN_Anim", true)
                    local animTween = nil
                    v.MouseButton1Click:Connect(function()
                        pcall(function()
                            -- 防重入：动画进行中跳过（不叠加 Tween）
                            if animTween and animTween.PlaybackState == Enum.PlaybackState.Playing then return end
                            local osz = v.Size
                            animTween = TweenService:Create(v, TweenInfo.new(0.05, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(osz.X.Scale * 0.94, osz.X.Offset * 0.94, osz.Y.Scale * 0.94, osz.Y.Offset * 0.94) })
                            animTween:Play()
                            task.delay(0.06, function()
                                pcall(function()
                                    animTween = TweenService:Create(v, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Size = osz })
                                    animTween:Play()
                                end)
                            end)
                        end)
                    end)
                elseif v:IsA("Frame") and not v:GetAttribute("NNVN_VAnim") then
                    v:SetAttribute("NNVN_VAnim", true)
                    local fadeTween = nil
                    v:GetPropertyChangedSignal("Visible"):Connect(function()
                        if v.Visible then
                            pcall(function()
                                if v.GroupTransparency == nil then return end
                                -- 防重入：先取消旧动画再淡入
                                if fadeTween and fadeTween.PlaybackState == Enum.PlaybackState.Playing then
                                    fadeTween:Cancel()
                                end
                                v.GroupTransparency = 0.3
                                fadeTween = TweenService:Create(v, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { GroupTransparency = 0 })
                                fadeTween:Play()
                            end)
                        end
                    end)
        ...
        for _, sg in pairs(game:GetService("CoreGui"):GetChildren()) do
            if sg:IsA("ScreenGui") then
                pcall(function() injectAnim(sg) end)
                -- 全局字体修正：所有文本统一 NotoSans（中文完美显示）
                pcall(function()
                    for _, obj in pairs(sg:GetDescendants()) do
                        if (obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox")) then
                            obj.Font = Enum.Font.NotoSans
                        end
                    end
                end)
            end
        ...
        if pg then
            for _, sg in pairs(pg:GetChildren()) do
                if sg:IsA("ScreenGui") and (tostring(sg.Name):find("NNVN") or tostring(sg.Name):find("Fluent") or tostring(sg.Name):find("WindUI")) then
                    pcall(function() injectAnim(sg) end)
                end
            end
        end
...
local Tabs = {
    ...
    AFKHub    = Window:AddTab({ Title = "⚡ 挂机中心", Icon = "" }),
}
...
fishSec:AddToggle("NNVN_AutoSkill", { Title = "▶ 自动使用钓法 --雪天缤纷", Default = false, Callback = function(v)
    getgenv().NNVN_AutoSkill = v
    if not v then RestoreTouchMode() end
end })
...
-- ====================================================
-- 9.5 自动回血（HP 低时自动使用回血技能，技能键可配）
-- ====================================================
autoHealSec = Tabs.Extra:AddSection("◆ 自动回血 --雪天缤纷")
autoHealSec:AddParagraph({
    Title = "说明 --雪天缤纷",
    Content = "血量低于阈值时自动按回血技能键(默认V)。游戏回血技能为服务器处理，客户端只能自动按，改不了数值"
})
getgenv().NNVN_AutoHeal = false
getgenv().NNVN_AutoHealThresh = 50
getgenv().NNVN_AutoHealKey = "V"
autoHealSec:AddToggle("NNVN_AutoHeal", {
    Title = "💗 自动回血(血量低自动按技能) --雪天缤纷",
    Default = false,
    Callback = function(v) getgenv().NNVN_AutoHeal = v end,
})
autoHealSec:AddSlider("NNVN_AutoHealThresh", {
    Title = "回血阈值(百分比) --雪天缤纷",
    Min = 10, Max = 90, Default = 50, Rounding = 1,
    Callback = function(v) getgenv().NNVN_AutoHealThresh = v end,
})
autoHealSec:AddDropdown("NNVN_AutoHealKey", {
    Title = "回血技能键 --雪天缤纷",
    Values = { "V", "C", "X", "Z" },
    Default = "V",
    Callback = function(v) getgenv().NNVN_AutoHealKey = v end,
})
task.spawn(function()
    local lastHealAt = 0
    while task.wait(0.5) do
        if not getgenv().NNVN_AutoHeal then continue end
        pcall(function()
            local char = LocalPlayer.Character
            ...
            local pct = hum.Health / maxHp * 100
            local now = os.clock()
            if pct <= getgenv().NNVN_AutoHealThresh and now - lastHealAt > 3 then
                lastHealAt = now
                local keyMap = {
                    Z = Enum.KeyCode.Z, X = Enum.KeyCode.X,
                    C = Enum.KeyCode.C, V = Enum.KeyCode.V,
                }
                local kc = keyMap[getgenv().NNVN_AutoHealKey or "V"]
                if kc then
                    VirtualInput:SendKeyEvent(true, kc, false, game)
                    task.wait(0.05)
                    VirtualInput:SendKeyEvent(false, kc, false, game)
                    if getgenv().NNVN_Notify then
                        WindUI:Notify{Title="自动回血 --雪天缤纷", Content="血量 "..string.format("%.0f%%", pct).." 低于阈值，已按 "..(getgenv().NNVN_AutoHealKey), Duration=2}
                    end
                end
            end
        end)
...
Tabs.Boss:AddToggle("NNVN_AutoPhase2", {
    ...
    Default  = false,
    Callback = function(v)
        getgenv().NNVN_AutoPhase2 = v
        if not v then RestoreTouchMode() end
    end,
})

bossGodSec = Tabs.Boss:AddSection("◆ 巨物战强化 --雪天缤纷")
bossGodSec:AddParagraph({
    Title = "ℹ️ 精准拉拔河 --雪天缤纷",
    Content = "白线自动锁定判定区中心，点击必然命中，拉拔河必定成功",
})
...
namelessBossSec:AddToggle("NNVN_AutoNamelessMinigame", {
    ...
    Default  = false,
    Callback = function(v)
        getgenv().NNVN_AutoNamelessMinigame = v
        if not v then RestoreTouchMode() end
    end,
})
namelessBossSec:AddSlider("NNVN_OctoSpeed", {
    Title    = "• 磨血速度 --雪天缤纷",
    Default  = 1, Min = 1, Max = 10, Rounding = 1,
    Callback = function(v) getgenv().NNVN_OctoSpeed = v end,
})
namelessBossSec:AddDropdown("NNVN_MinigameMode", {
    Title    = "◉ 通关模式 --雪天缤纷",
    Values   = {"模拟点击", "智能直过"},
    Default  = "模拟点击",
    Callback = function(v)
        getgenv().NNVN_MinigameMode = (v == "智能直过") and 2 or 1
    end,
})
...
-- ====================================================
-- 任务翻译：获取当前所有任务 + 选择 + 翻译成中文
-- ====================================================
questTransSec = Tabs.Quest:AddSection("◆ 任务翻译 --雪天缤纷")
questListCache = {}
questTransP = questTransSec:AddParagraph({ Title = "任务列表 --雪天缤纷", Content = "点击下方「刷新任务」获取当前任务" })
questTransSec:AddButton({
    Title = "🔄 刷新任务(读取当前所有任务) --雪天缤纷",
    Callback = function()
        pcall(function()
            questListCache = {}
            local lines = {}
            local idx = 0
            for _, diff in ipairs({ "Easy", "Hard" }) do
                local prog = getgenv().NNVN_GetQuestProgress and getgenv().NNVN_GetQuestProgress(diff)
                if prog then
                    for _, q in ipairs(prog) do
                        idx = idx + 1
                        table.insert(questListCache, { diff = diff, name = q.name, cur = q.current, total = q.total, done = q.completed })
                        table.insert(lines, string.format("%d) [%s%s] %s (%d/%d)",
                            idx, diff, q.completed and "✓" or "", q.name, q.current, q.total))
                    end
                end
            end
            if idx == 0 then
                table.insert(lines, "当前没有任务（先接任务再来刷新）")
            end
            if questTransP then questTransP:SetContent(table.concat(lines, "\n")) end
            if getgenv().NNVN_Notify then
                WindUI:Notify{Title="任务翻译 --雪天缤纷", Content="找到 "..idx.." 个任务，输入编号翻译", Duration=3}
            end
        end)
    end,
})
questTransSec:AddInput({
    Title = "任务编号(看上表) --雪天缤纷",
    Placeholder = "1",
    Value = "1",
    Callback = function(v)
        local n = tonumber(v)
        if n then questSelIdx = n end
    end,
})
questSelIdx = 1
questTransSec:AddButton({
    Title = "🌐 翻译所选任务 --雪天缤纷",
    Callback = function()
        local q = questListCache[questSelIdx]
        if not q then
            if getgenv().NNVN_Notify then WindUI:Notify{Title="任务翻译 --雪天缤纷", Content="先点刷新任务", Duration=2} end
            return
        end
        local translated = TranslateQuestDesc(q.name)
        local line = string.format("[%s任务] %s\n📖 翻译: %s\n进度: %d/%d %s",
            q.diff, q.name, translated, q.cur, q.total, q.done and "✅已完成" or "⏳进行中")
        if questTransP then questTransP:SetContent(line) end
        if getgenv().NNVN_Notify then
            WindUI:Notify{Title="📖 任务翻译 --雪天缤纷", Content=translated, Duration=5}
        end
    end,
})

-- 任务翻译引擎（规则解析 + 鱼名/饵料汉化表）
function TranslateQuestDesc(desc)
    if not desc or desc == "" then return "(空)" end
    -- 提取动词和数量
    local verb = desc:match("^(%a+)") or ""
    local count = tonumber(desc:match("(%d+)"))
    -- 提取目标名（优先匹配汉化表里的长鱼名）
    local FISH_T = getgenv().NNVN_FISH_CN or {}
    local BAIT_T = getgenv().NNVN_BAIT_CN or {}
    local target, targetCN = nil, nil
    for fname, fcn in pairs(FISH_T) do
        if desc:find(fname, 1, true) then target, targetCN = fname, fcn break end
    end
    if not target then
        for bname, bcn in pairs(BAIT_T) do
            if desc:find(bname, 1, true) then target, targetCN = bname, bcn break end
        end
    end
    if not target then
        -- 尝试提取引号/大写单词
        target = desc:match("([A-Z][A-Za-z ]+)$")
        targetCN = target
    end
    local verbMap = {
        Catch = "钓到", CatchUp = "追上", Reach = "达到", Sell = "出售", Use = "使用",
        Upgrade = "升级", Win = "赢得", Buy = "购买", Equip = "装备", Unlock = "解锁",
        Defeat = "击败", Claim = "领取", Craft = "制作", Open = "打开", Collect = "收集",
    }
    local verbCN = verbMap[verb] or verb
    local countPart = count and (" " .. count .. " 条") or ""
    local targetPart = (targetCN or target or desc)
    if verb == "Catch" then
        return "🎣 钓到" .. countPart .. "「" .. targetPart .. "」"
    elseif verb == "Sell" then
        return "💰 出售" .. countPart .. "「" .. targetPart .. "」"
    elseif verb == "Use" then
        return "🪤 使用" .. countPart .. "「" .. targetPart .. "」"
    elseif verb == "Reach" then
        return "📏 达到" .. countPart .. "「" .. targetPart .. "」"
    elseif verb == "Defeat" then
        return "⚔ 击败" .. countPart .. "「" .. targetPart .. "」"
    elseif verb == "Craft" then
        return "🧪 制作" .. countPart .. "「" .. targetPart .. "」"
    end
    return "🔧 " .. verbCN .. countPart .. "「" .. targetPart .. "」"
end
                        ...
                        for _, npc in ipairs(godFolder:GetChildren()) do
                            ...
                            if npc:IsA("Model") then
                                ...
                                if not alreadyPrayed then
                                    ...
                                    task.wait(1.5)
                                    if getgenv().NNVN_Notify then
                                        WindUI:Notify{Title="神灵 --雪天缤纷", Content="已向 "..npc.Name.." 祈祷", Duration=3}
                                    end
                                else
                                    if getgenv().NNVN_Notify then
                                        WindUI:Notify{Title="神灵 --雪天缤纷", Content=npc.Name.." 今天已祈祷过", Duration=2}
                                    end
                                end
                                task.wait(0.5)
                            end
...
bannerSec:AddToggle("NNVN_AutoGacha", {
    ...
    Callback = function(v)
        ...
        if v then
                    ...
                    pcall(function()
                        ...
                        if ticketCount >= needed then
                            ...
                            if crystalCount >= crystalsNeeded then
                                ...
                                WindUI:Notify{Title="抽卡 --雪天缤纷", Content="门票/水晶不足！需要 "..crystalsNeeded.." (现有 "..crystalCount.."). Stopping.", Duration=4}
                                getgenv().NNVN_AutoGacha = false
                            end
                        end
                        if canRoll then
                            ...
                            WindUI:Notify{Title="抽卡 --雪天缤纷", Content="已抽取 "..selectedGachaAmount.."x "..selectedBanner, Duration=2}
                        end
                    end)
                    task.wait(2)
                end
            ...
            if gachaCo then coroutine.close(gachaCo); gachaCo = nil end
            WindUI:Notify{Title="抽卡 --雪天缤纷", Content="已停止", Duration=2}
        end
    end
})
...
-- ============ 显示层汉化映射（查找键仍用英文，仅 UI 显示中文） ============
local ISLE_CN = {
    ["Beginning Isle"]="初始岛", ["Bamboo Isle"]="竹岛", ["Fallout Isle"]="辐射岛",
    ["Sovereign Isle"]="主权岛", ["Perch Isle"]="鲈鱼岛", ["Frost Isle"]="霜冻岛",
    ["Coconut Isle"]="椰岛", ["Amber Isle"]="琥珀岛", ["Battlefield Isle"]="战场岛",
    ["Mistpeak Isle"]="雾峰岛",
}
local SELLER_CN = {
    ["Beginning Isle's Rod Seller"]="初始岛鱼竿商人", ["Bamboo Isle's Rod Seller"]="竹岛鱼竿商人",
    ["Fallout Isle's Rod Seller"]="辐射岛鱼竿商人", ["Sovereign Isle's Rod Seller"]="主权岛鱼竿商人",
    ["Frost Isle's Rod Seller"]="霜冻岛鱼竿商人", ["Coconut Isle's Rod Dealer"]="椰岛鱼竿贩子",
    ["Amber Isle's Rod Seller"]="琥珀岛鱼竿商人",
}
local ROD_CN = {
    ["Bamboo Rod"]="竹竿", ["Iron Hook Rod"]="铁钩竿", ["Steel Rod"]="钢竿",
    ["Enchanted Steel Rod"]="附魔钢竿", ["Alloy Rod"]="合金竿", ["Emerald Rod"]="翡翠竿",
    ["Bloodfire Rod"]="血火竿", ["Triple Steel Rod"]="三重钢竿", ["Golden Rod"]="黄金竿",
    ["Grandmaster Steel Rod"]="宗师钢竿", ["Grandmaster Emerald Rod"]="宗师翡翠竿",
    ["Grandmaster Golden Rod"]="宗师黄金竿", ["Inferno Rod"]="地狱火竿",
    ["Steel Spine Rod"]="钢脊竿", ["Golden Spine Rod"]="黄金脊竿", ["Platinum Spine Rod"]="白金脊竿",
    ["Diamond Spine Rod"]="钻石脊竿", ["Gravisteel Rod"]="引力钢竿", ["Auric Gravity Rod"]="黄金引力竿",
    ["Inferno Gravity Rod"]="地狱引力竿", ["Cryo Gravity Rod"]="冰霜引力竿",
    ["Thunder Thorn Rod"]="雷霆荆棘竿", ["Soul Thorn Rod"]="灵魂荆棘竿", ["Starlight Rod"]="星光竿",
}
local SKILL_CN = {
    ["Skyfall Stomp"]="天坠践踏", ["Infinite Sky Ascension"]="无限登天", ["Unshake Stance"]="不动姿态",
    ["Swift Reel"]="疾速收线", ["Demonfall Technique"]="魔坠之术", ["Reel Machine"]="收线机器",
    ["Reel Sprint"]="收线冲刺", ["Dragonfish Vitality"]="龙鱼之息", ["Skybreaker Technique"]="破天之术",
    ["Aerial Rod Throw"]="凌空抛竿", ["Thousand Enemy Sweep"]="千敌横扫", ["One-Strike Heaven Gate"]="一击天门",
    ["Rolling Chaos"]="翻滚混沌", ["Rooster Strike"]="鸡拳", ["Phoenix Strike Art"]="凤击之术",
}
local BAIT_CN = {
    ["Basic Bait"]="基础鱼饵", ["Crude Mash Bait"]="粗制混合饵", ["Corrupted Essence Bait"]="腐蚀精华饵",
    ["Elite Bait"]="精英鱼饵", ["Ancestral Bait"]="始祖鱼饵",
}
local SECRET_ROD_CN = {
    ["Ascendant Bamboo Rod"]="飞升竹竿", ["Blazeshark Rod"]="烈焰鲨竿", ["Lifebloom Rod"]="生命花竿",
    ["Anchorbound Rod"]="锚缚竿", ["Demonic Rod"]="恶魔竿", ["Kraken Rod"]="海妖竿",
}
local FISH_CN = {
    ["Colossal Tigerfish"]="巨型虎鱼", ["Heavenpiercer Turtle"]="破天龟", ["Golden Guardian Fish"]="黄金守护鱼",
    ["Crimson Electric Eel"]="绯红电鳗", ["Frost Kingfish"]="霜冻王鱼", ["Ascended Perch"]="飞升鲈鱼",
    ["Primordial Kunfish Overlord"]="太古坤鱼霸主", ["Warbringer Shark"]="战祸鲨",
}
getgenv().NNVN_FISH_CN = FISH_CN
getgenv().NNVN_BAIT_CN = BAIT_CN
getgenv().NNVN_ISLE_CN = ISLE_CN
getgenv().NNVN_SKILL_CN = SKILL_CN
-- ============ 映射表结束 ============

TeleportList = {
    {name="Beginning Isle", cn="初始岛",  CFrame.new(-213.939972,6.76193953,33.2846527)},
    {name="Bamboo Isle", cn="竹岛",     CFrame.new(-1184.15674,10.6356201,-84.3372498)},
    {name="Fallout Isle", cn="辐射岛",    CFrame.new(-6.55301952,6.24169779,1185.16528)},
    {name="Sovereign Isle", cn="主权岛",  CFrame.new(-1248.17249,7.31857586,1208.54199)},
    {name="Perch Isle", cn="鲈鱼岛",      CFrame.new(-61.9632416,-8.2120657,-1310.39124)},
    {name="Frost Isle", cn="霜冻岛",      CFrame.new(-1328.79614,7.63562012,-1488.55383)},
    {name="Coconut Isle", cn="椰岛",    CFrame.new(1429.93628,7.63562012,-1478.6311)},
    {name="Amber Isle", cn="琥珀岛",      CFrame.new(1239.81299,10.6663065,1335.96521)},
    {name="Battlefield Isle", cn="战场岛",CFrame.new(1394.1842,9.27561855,200.794785)},
    {name="Mistpeak Isle", cn="雾峰岛",   CFrame.new(2611.62134,9.27561855,-81.6781845)},
}
...
for _, island in ipairs(TeleportList) do
    -- 修复：TeleportList 数据里 CFrame 是数组元素(island[1])，补 cf 字段供全脚本使用
    island.cf = island.cf or island[1]
    matIslandNameToIsland[island.name] = island
end
...
function matTeleportToIsland(islandName)
    ...
    if hrp then
        local target = CFrame.new(island.cf.Position) + Vector3.new(0,5,0)
        hrp.CFrame = target
        -- 防回弹
        task.spawn(function()
            for _ = 1, 8 do
                task.wait(0.03)
                pcall(function()
                    if hrp and hrp.Parent then hrp.CFrame = target end
                end)
            end
        end)
        return true
...
matSec:AddToggle("NNVN_MaterialFarming", {
    ...
    Callback = function(v)
        ...
        if v then
                ...
                while getgenv().NNVN_MaterialFarming do
                    pcall(function()
                        ...
                        if req then
                            ...
                            if not ok then
                                materialCurrentTarget = nil
                                WindUI:Notify{Title="材料刷取 --雪天缤纷", Content="需要 "..(req.label or "特殊天气").." 才能钓 "..(FISH_CN[fishName] or fishName).."，等待中...", Duration=3}
                                task.wait(5)
                                return
                            end
            ...
            matDisableFishing()
            materialCurrentTarget = nil
            WindUI:Notify{Title="材料刷取 --雪天缤纷", Content="已停止", Duration=2}
        end
    end
})
...
for _, si in ipairs(sortedSellers) do
    local sec = Tabs.BuyRod:AddSection("◆ "..(SELLER_CN[si.seller] or si.seller))
    for _, rod in ipairs(rodGroups[si.seller]) do
        local rodRef = rod   -- capture
        sec:AddButton({
            Title    = "▣ "..(ROD_CN[rodRef.name] or rodRef.name).." - $"..formatPrice(rodRef.price),
            ...
            Callback = function()
                task.spawn(function()
                    ...
                    if getgenv().NNVN_Notify then
                        WindUI:Notify{Title="购买鱼竿 --雪天缤纷", Content="已购买 "..(ROD_CN[rodRef.name] or rodRef.name), Duration=2}
                    end
                end)
            end
        })
...
for _, rod in ipairs(secretRodData) do
    local rodRef = rod
    secSec:AddButton({
        Title    = "▣ "..(SECRET_ROD_CN[rodRef.name] or rodRef.name).." - "..formatPrice(rodRef.price).." 宝石",
...
for _, s in ipairs(skillData) do
    table.insert(skillDisplayNames, (SKILL_CN[s.name] or s.name).." - $"..formatPrice(s.price))
end
...
learnSec:AddDropdown("SkillSelection", {
    ...
    Callback = function(v)
        for _, s in ipairs(skillData) do
            if v == (SKILL_CN[s.name] or s.name).." - $"..formatPrice(s.price) then selSkillName = s.name; break end
        end
    end
})
learnSec:AddButton({
    ...
    Callback = function()
        task.spawn(function()
            ...
            if getgenv().NNVN_Notify then
                WindUI:Notify{Title="钓法 --雪天缤纷", Content="已购买 "..(SKILL_CN[selSkillName] or selSkillName), Duration=2}
            end
        end)
    end
})
...
baitDisplayNames = {}
for _, b in ipairs(baitData) do table.insert(baitDisplayNames, (BAIT_CN[b.name] or b.name).." - $"..formatPrice(b.price)) end
...
function addBillboard(parent, text, color, offset)
    ...
    lbl.Text      = text
    lbl.Font      = Enum.Font.NotoSans; lbl.FontWeight = Enum.FontWeight.Bold
    lbl.TextScaled = true
    return bb
end
...
-- ====================================================
-- ====================================================
-- 17.5 TAB: 挂机中心（一键挂机 + 收益统计 + 智能卖鱼）
-- ====================================================
hubSec = Tabs.AFKHub:AddSection("◆ 一键挂机 --雪天缤纷")
hubSec:AddParagraph({
    Title = "使用说明 --雪天缤纷",
    Content = "一键开启完整挂机循环：自动抛竿→收鱼→卖鱼→买饵→接任务→交任务→防挂机。关闭时恢复之前设置"
})
getgenv().NNVN_AutoAFK = false
local hubPrevStates = {}
hubSec:AddToggle("NNVN_AutoAFK", {
    Title = "🔄 一键挂机(全自动循环) --雪天缤纷",
    Default = false,
    Callback = function(v)
        getgenv().NNVN_AutoAFK = v
        if v then
            -- 保存当前状态
            local keys = { "NNVN_AutoCast", "NNVN_AutoSell", "NNVN_AutoBuyBait", "NNVN_AutoAcceptQuest", "NNVN_AutoTurnIn", "NNVN_AutoTicket", "NNVN_AntiAFK" }
            hubPrevStates = {}
            for _, k in ipairs(keys) do hubPrevStates[k] = getgenv()[k] end
            -- 开启完整循环
            getgenv().NNVN_AutoCast        = true
            getgenv().NNVN_AutoSell        = true
            getgenv().NNVN_AutoBuyBait     = true
            getgenv().NNVN_AutoAcceptQuest = true
            getgenv().NNVN_AutoTurnIn      = true
            getgenv().NNVN_AutoTicket      = true
            getgenv().NNVN_AntiAFK         = true
            if getgenv().NNVN_Notify then WindUI:Notify{Title="挂机中心 --雪天缤纷", Content="✅ 一键挂机已开启（抛竿+卖鱼+买饵+任务+防挂机）", Duration=3} end
        else
            -- 恢复之前状态
            for k, old in pairs(hubPrevStates) do getgenv()[k] = old end
            if getgenv().NNVN_Notify then WindUI:Notify{Title="挂机中心 --雪天缤纷", Content="已停止挂机，恢复之前设置", Duration=2} end
        end
    end,
})
hubStatusP = hubSec:AddParagraph({ Title = "挂机状态 --雪天缤纷", Content = "未开启" })

-- 挂机状态刷新（显示当前各环节状态）
task.spawn(function()
    while task.wait(2) do
        if not getgenv().NNVN_AutoAFK then continue end
        pcall(function()
            local parts = {}
            local map = {
                NNVN_AutoCast = "抛竿", NNVN_AutoSell = "卖鱼", NNVN_AutoBuyBait = "买饵",
                NNVN_AutoAcceptQuest = "接任务", NNVN_AutoTurnIn = "交任务", NNVN_AntiAFK = "防挂机",
            }
            for k, name in pairs(map) do
                if getgenv()[k] then table.insert(parts, name) end
            end
            if hubStatusP then hubStatusP:SetContent("运行中: " .. table.concat(parts, " → ")) end
        end)
    end
end)

-- ====================================================
-- 收益统计（自动识别钱字段，实时统计总收入）
-- ====================================================
profitSec = Tabs.AFKHub:AddSection("◆ 收益统计 --雪天缤纷")
profitStat = { total = 0, count = 0, startAt = os.clock(), lastValue = nil, lastDelta = 0 }
local function FindMoneyValue()
    local ls = LocalPlayer:FindFirstChild("Leaderstats")
    if not ls then return nil end
    -- 优先名字匹配
    for _, c in pairs(ls:GetChildren()) do
        if (c:IsA("NumberValue") or c:IsA("IntValue")) and c.Name:lower():find("cash") or (c:IsA("NumberValue") or c:IsA("IntValue")) and c.Name:lower():find("money") or (c:IsA("NumberValue") or c:IsA("IntValue")) and c.Name:lower():find("coin") or (c:IsA("NumberValue") or c:IsA("IntValue")) and c.Name:find("钱") or (c:IsA("NumberValue") or c:IsA("IntValue")) and c.Name:find("金") then
            return c
        end
    end
    -- 兜底：第一个非 Level 数值字段
    for _, c in pairs(ls:GetChildren()) do
        if (c:IsA("NumberValue") or c:IsA("IntValue")) and not c.Name:lower():find("level") and not c.Name:find("等级") then
            return c
        end
    end
    return nil
end
profitP = profitSec:AddParagraph({ Title = "统计 --雪天缤纷", Content = "等待数据..." })
task.spawn(function()
    while task.wait(3) do
        pcall(function()
            local mv = FindMoneyValue()
            if not mv then
                if profitP then profitP:SetContent("未找到金币字段（Leaderstats）") end
                return
            end
            local cur = mv.Value
            if profitStat.lastValue and cur > profitStat.lastValue then
                local gain = cur - profitStat.lastValue
                profitStat.total = profitStat.total + gain
                profitStat.count = profitStat.count + 1
                profitStat.lastDelta = gain
            end
            profitStat.lastValue = cur
            local elapsed = os.clock() - profitStat.startAt
            local perMin = elapsed > 60 and (profitStat.total / (elapsed / 60)) or (profitStat.count > 0 and profitStat.total or 0)
            if profitP then
                profitP:SetContent(string.format(
                    "当前金币: %s\n本局收益: %s  (共 %d 次)\n平均: %s/分钟\n运行: %.1f 分钟",
                    tostring(math.floor(cur)), tostring(math.floor(profitStat.total)),
                    profitStat.count, tostring(math.floor(perMin)), elapsed / 60
                ))
            end
        end)
    end
...
-- 智能卖鱼：保留指定鱼种（先收藏再全卖）
-- ====================================================
smartSec = Tabs.AFKHub:AddSection("◆ 智能卖鱼 --雪天缤纷")
smartSec:AddParagraph({
    Title = "原理 --雪天缤纷",
    Content = "填要保留的鱼名（如 Ancestral Shark），开启后自动先收藏再卖全部 → 保留的鱼保住，其余卖掉"
})
getgenv().NNVN_KeepFishName = ""
smartSec:AddInput({
    Title = "保留的鱼名(留空=全卖) --雪天缤纷",
    Placeholder = "Ancestral Shark",
    Value = "",
    Callback = function(v) getgenv().NNVN_KeepFishName = v or "" end,
})
getgenv().NNVN_SmartSell = false
smartSec:AddToggle("NNVN_SmartSell", {
    Title = "▶ 智能卖鱼 --雪天缤纷",
    Default = false,
    Callback = function(v) getgenv().NNVN_SmartSell = v end,
})
task.spawn(function()
    while task.wait(4) do
        if not getgenv().NNVN_SmartSell then continue end
        pcall(function()
            local keep = getgenv().NNVN_KeepFishName or ""
            -- 先收藏保留的鱼（如果填了）
            if keep ~= "" then
                local data = ReplicatedStorage:FindFirstChild("Data")
                local pd = data and data:FindFirstChild(tostring(LocalPlayer.UserId))
                ...
                if inv and fev then
                    for _, item in ipairs(inv:GetChildren()) do
                        if item.Name:sub(1, #keep) == keep and not item.Name:match(" | Favorite$") then
                            fev:FireServer(item.Name)
                            task.wait(0.1)
                        end
                    end
...
utilSec:AddSlider("NNVN_WalkSpeed", {
    Title = "◉ 移动速度 --雪天缤纷", Default = 16, Min = 8, Max = 250, Rounding = 1,
    Callback = function(v)
...
spinSec:AddSlider("NNVN_SpinnerSpeed", {
    Title = "• 旋转速度(度/秒) --雪天缤纷",
    Default = 500, Min = 60, Max = 3000, Rounding = 1,
    Callback = function(v) getgenv().NNVN_SpinnerSpeed = v end,
})

worldSec = Tabs.Extra:AddSection("◆ 世界 --雪天缤纷")
worldSec:AddToggle("NNVN_LockStamina", {
    Title = "▶ 锁定体力(满值) --雪天缤纷", Default = false,
    Callback = function(v) getgenv().NNVN_LockStamina = v end,
})
...
-- BGM player
-- 雨爱候选 ID（原曲可能被版权下架，Nightcore 版兜底；用户自定义 ID 优先）
local BGM_CANDIDATES = { "4727843133" }
local function tryPlayBGM(idx)
    pcall(function()
        local cur = getgenv().NNVN_BGM_Sound
        if cur then pcall(function() cur:Stop() end); cur:Destroy(); getgenv().NNVN_BGM_Sound = nil end

        -- 构建候选链：用户 ID 优先，备用 ID 兜底
        local ids = {}
        local user = getgenv().NNVN_BGM_SongId or ""
        if tostring(user) ~= "" and tostring(user):match("%d+") then
            table.insert(ids, tostring(user):match("%d+"))
        end
        for _, c in ipairs(BGM_CANDIDATES) do
            local dup = false
            for _, e in ipairs(ids) do if e == c then dup = true end end
            if not dup then table.insert(ids, c) end
        end
        if idx > #ids then
            if getgenv().NNVN_Notify then
                WindUI:Notify{Title="BGM --雪天缤纷", Content="歌曲全部加载失败，请在设置里更换歌曲ID", Duration=4}
            end
            return
        end
        ...
        -- 加载检测：8 秒内未加载成功 → 自动切换下一个候选
        task.delay(8, function()
            if s ~= getgenv().NNVN_BGM_Sound then return end
            if s.TimeLength <= 0 then
                if getgenv().NNVN_Notify then
                    WindUI:Notify{Title="BGM --雪天缤纷", Content="歌曲 "..id.." 加载失败，尝试下一个...", Duration=3}
                end
                task.spawn(function() tryPlayBGM(idx + 1) end)
            elseif getgenv().NNVN_Notify then
                WindUI:Notify{Title="BGM --雪天缤纷", Content="正在播放 rbxassetid://"..id, Duration=3}
            end
        end)
    end)
end
...
Wf.TextColor3 = Color3.fromRGB(205,180,255); Wf.Text = " ◆ 雪天中心 v1.2 Beta [WindUI]  雪天缤纷Pro"
Wf.Font = Enum.Font.NotoSans; Wf.TextSize = 13
...
-- 抛竿朝向：陀螺开启时使用基准朝向，保证抛竿始终朝前不受旋转影响
local function GetCastCFrame(root)
    local base = getgenv().NNVN_SpinnerBase
    if not base then return root.CFrame end
    local look = base.LookVector * Vector3.new(1,0,1)
    if look.Magnitude < 0.01 then look = Vector3.new(0,0,-1) end
    return CFrame.lookAt(root.Position, root.Position + look.Unit)
end
...
local function castRodBossFarm()
    ...
    pcall(function()
        ...
        if root then
            ReplicatedStorage.Events.Fishing:FireServer(GetCastCFrame(root))
        else
            ReplicatedStorage.Events.Fishing:FireServer()
        end
    end)
...
-- 锚点修复：动态查找硬度条（游戏结构变化也不怕）+ 信号提频
local anchorNextFireAt = 0
local function FindAnchorBar(fb)
    -- 1. 原路径
    local bf = fb:FindFirstChild("BarFrame")
    if bf then
        local b = bf:FindFirstChild("Bar") or bf:FindFirstChildOfClass("Frame")
        if b then return b end
    end
    -- 2. 直接子级 Bar
    local b = fb:FindFirstChild("Bar")
    if b then return b end
    -- 3. 兜底：找最宽的横条 Frame（硬度条特征：宽 > 高，非按钮）
    local best, bestW = nil, 0
    for _, v in pairs(fb:GetDescendants()) do
        if v:IsA("Frame") and v.AbsoluteSize.X > 60 and v.AbsoluteSize.X > v.AbsoluteSize.Y * 2 and not v:IsA("TextButton") then
            local w = v.AbsoluteSize.X
            if w > bestW then best, bestW = v, w end
        end
    end
    return best
end
RunService.RenderStepped:Connect(function()
    ...
    pcall(function()
        ...
        if fb.Visible then
            -- 硬度条锁定中间（客户端 UI 操作，动态查找）
            local bar = FindAnchorBar(fb)
            if bar then
                bar.Position = UDim2.new(0.5, 0, bar.Position.Y.Scale, 0)
            end
            -- 信号：0.12s 一次（原版每帧，折中防 UI 卡死）
            local now = os.clock()
            if now >= anchorNextFireAt then
                anchorNextFireAt = now + 0.12
...
local function handleNamelessRhythmMinigame()
    ...
    for laneName, keyLetter in pairs(namelessLaneMap) do
        ...
        if noteFrame and button and noteFrame.Visible and button.Visible then
            ...
            -- 判定窗口：70% 按钮高（原版）+ 延迟补偿
            -- 非对称：音符在上方(未到)时提前触发（补偿点击到服务器生效的延迟），
            -- 下方(已过)窗口缩小（防误触扣血）
            local hitWindow = math.max(24, math.floor(button.AbsoluteSize.Y * 0.7))
            local leadPx = math.max(6, math.floor(button.AbsoluteSize.Y * 0.18)) -- 提前量
            ...
            for _, child in ipairs(noteFrame:GetChildren()) do
                if child:IsA("GuiObject") and child.Visible then
                    ...
                    -- 音符中心在判定窗口内 → 按键 + 点击 + 命中信号
                    local dist = buttonCenterY - noteCenterY -- 正=音符在上方(未到)
                    if dist >= -hitWindow and dist <= hitWindow + leadPx then
                        if isTouchDevice then
                            -- 手机：触摸点击按钮已触发轨道（跳过键盘，避免切桌面模式删摇杆/钓鱼键）
                            SmartClick(button)
                        else
                            pressRhythmKey(keyLetter)
                            clickGuiButton(button)
                        end
                        pcall(function() ReplicatedStorage.Events.RhythmHit:FireServer("hit") end)
                        break
                    end
                end
            end
    ...
    -- 心跳 RhythmHit：推进度（速度滑块控制频率，1倍=原版0.08s，10倍=0.008s）
    if now >= namelessNextRemoteAt then
        local octoSpeed = getgenv().NNVN_OctoSpeed or 1
        if octoSpeed < 1 then octoSpeed = 1 elseif octoSpeed > 10 then octoSpeed = 10 end
        namelessNextRemoteAt = now + (0.08 / octoSpeed)
        pcall(function()
...
-- 帧率无关扫描：低帧率手机也能保持高频检测（RenderStepped 帧率低会漏音符）
task.spawn(function()
...
-- 锁定体力：全类型全位置扫描 + 持续本地锁定（不是假回满，而是高频强制满值）
-- 兼容 Leaderstats / 玩家数据 / 角色 Value / 角色 Attributes / 任意嵌套文件夹
local staminaKnownMax = {} -- 无 Max 属性时的满值基准（记录历史最高观察值）
local function isStaminaName(nm)
    if type(nm) ~= "string" then return false end
    local lo = nm:lower()
    return lo:find("stamina") or lo:find("energy") or lo:find("vitality")
        or lo:find("health") or lo:find("mana") or lo:find("hp")
        or lo:find("体力") or lo:find("耐力") or lo:find("精力")
        or lo:find("血量") or lo:find("血条") or lo:find("饱食") or lo:find("饥饿")
        or lo:find("durability") or lo:find("tension") or lo:find("linehealth")
        or lo:find("fishhealth") or lo:find("rodhp") or lo:find("power")
end
-- 已被 Changed 事件挂钩的体力值（避免重复挂钩）
local staminaHooked = {}
-- 锁定记录：属性名 → 锁定的满值（用于通知）
local staminaLocked = {}
-- 掉血目标自动发现：钓鱼时监控所有候选值，谁在持续掉血就锁定谁（不靠名字猜）
local staminaWatch = {}   -- v -> {prev=, at=}
local staminaTargets = {} -- v -> true（确认的掉血目标，名字不匹配也锁）
local function TryLockOne(v)
    if not v or not v.Parent then return end
    local maxV = v.Parent:FindFirstChild("Max"..v.Name) or v.Parent:FindFirstChild(v.Name.."Max")
        or v.Parent:FindFirstChild("max_"..v.Name) or v.Parent:FindFirstChild(v.Name.."_max")
    if staminaTargets[v] and not maxV and staminaKnownMax[v] then
        -- 确认的掉血目标且无 Max：锁历史最高（已记录）
        if v.Value < staminaKnownMax[v] then v.Value = staminaKnownMax[v] end
        staminaLocked[v.Name] = staminaKnownMax[v]
        return
    end
    if maxV and (maxV:IsA("NumberValue") or maxV:IsA("IntValue")) then
        -- 有 Max：直接锁满（Max 实时读取——随等级提升自动锁新满值）
        if v.Value < maxV.Value then v.Value = maxV.Value end
        staminaLocked[v.Name] = maxV.Value
        -- 监听 Max 变化：等级提升瞬间，立即用新满值锁定（不等轮询）
        if not staminaHooked[maxV] then
            staminaHooked[maxV] = true
            maxV.Changed:Connect(function()
                if not getgenv().NNVN_LockStamina then return end
                pcall(function()
                    if v and v.Parent and maxV and maxV.Parent then
                        if v.Value < maxV.Value then v.Value = maxV.Value end
                        staminaLocked[v.Name] = maxV.Value
                    end
                end)
        ...
        -- 无 Max：锁定历史最高观察值——等级提升导致满值跳升时，max() 自动捕捉新满值
        local cur = v.Value
        staminaKnownMax[v] = math.max(staminaKnownMax[v] or cur, cur)
        if v.Value < staminaKnownMax[v] then v.Value = staminaKnownMax[v] end
        staminaLocked[v.Name] = staminaKnownMax[v]
    end
    -- 真实锁定：挂 Changed 事件，服务器/游戏一拉低立即反击拉回（快于轮询）
    if not staminaHooked[v] then
        staminaHooked[v] = true
...
-- 钓鱼拉扯掉血监控：发现正在下降的数值 → 标记为锁定目标
local function watchFishingDrain()
    if not getgenv().NNVN_LockStamina then return end
    -- 是否在钓鱼中
    local fishing = false
    pcall(function()
        local char = LocalPlayer.Character
        fishing = (char and char:GetAttribute("Fishing")) or false
        if not fishing and MainGui then
            local f = MainGui:FindFirstChild("Fishing")
            fishing = f and f.Visible or false
        end
    end)
    if not fishing then
        staminaWatch = {}
    ...
    pcall(function()
        ...
        local function observe(v)
            if not v or not v.Parent then return end
            local val = v.Value
            -- 值过滤：只观察小数值（体力/血量级别），排除钱/经验等大数值防误锁
            if val > 100000 then return end
            local w = staminaWatch[v]
            if not w then
                staminaWatch[v] = { prev = val, at = now }
            else
                if val < w.prev then
                    -- 值下降了：确认掉血目标（钓鱼中持续掉 = 体力/血量）
                    staminaTargets[v] = true
                end
                w.prev = val
                w.at = now
            end
        ...
        scanTree(LocalPlayer:FindFirstChild("Leaderstats"))
        scanTree(ReplicatedStorage.Data:FindFirstChild(tostring(LocalPlayer.UserId)))
        local char = LocalPlayer.Character
        if char then
            for name, val in pairs(char:GetAttributes()) do
                if typeof(val) == "number" and isStaminaName(name) then
                    -- Attribute 监控（简化：匹配即观察峰值）
                end
            end
            scanTree(char)
        end
...
local function TryLockStamina()
    if not getgenv().NNVN_LockStamina then return end
    watchFishingDrain() -- 钓鱼掉血监控：发现目标后立即锁定
    pcall(function()
        -- 1. Leaderstats（含嵌套）
        local ls = LocalPlayer:FindFirstChild("Leaderstats")
        if ls then
            for _, v in ipairs(ls:GetDescendants()) do
                if (v:IsA("NumberValue") or v:IsA("IntValue") or v:IsA("IntConstrainedValue")) and (isStaminaName(v.Name) or staminaTargets[v]) then
                    TryLockOne(v)
                end
            end
        end
        ...
        if char then
            ...
            for name, val in pairs(char:GetAttributes()) do
                if typeof(val) == "number" and isStaminaName(name) then
                    local maxVal = char:GetAttribute("Max"..name) or char:GetAttribute(name.."Max")
                    if type(maxVal) == "number" and val < maxVal then
                        char:SetAttribute(name, maxVal)
                    end
                end
            end
        end
        -- 3.5 角色 Humanoid 血量（钓鱼拉扯若扣的是角色血，一并锁满）
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                local hmax = hum.MaxHealth
                if hum.Health < hmax then hum.Health = hmax end
                staminaLocked["Humanoid.Health"] = hmax
            end
        end
        -- 4. PlayerGui / 其他玩家层级数值（部分游戏放这里）
        local pg = LocalPlayer:FindFirstChild("PlayerGui")
        if pg then
            for _, v in ipairs(pg:GetDescendants()) do
                if (v:IsA("NumberValue") or v:IsA("IntValue")) and isStaminaName(v.Name) and v.Parent and v.Parent:IsA("Folder") then
                    TryLockOne(v)
                end
            end
        end
...
local staminaNotified = false
task.spawn(function()
    while task.wait(0.02) do -- 极限高频锁定 + Changed 事件双保险，服务器同步也拉不下来
        TryLockStamina()
        -- 锁定成功后通知一次（告知锁定了哪个属性）
        if not staminaNotified and getgenv().NNVN_LockStamina and next(staminaLocked) then
            staminaNotified = true
            local name, maxv = next(staminaLocked)
            if name and maxv and getgenv().NNVN_Notify then
                task.delay(3, function()
                    -- 完整路径诊断：显示属性在哪（如 Leaderstats.HP）
                    local full = tostring(name)
                    pcall(function()
                        for k, nm in pairs(staminaLocked) do
                            if k ~= name and tostring(k) == tostring(name) then end
                        end
                    end)
                    local path = full
                    pcall(function()
                        for v, mv in pairs(staminaLocked) do
                            if mv == maxv and v ~= name then end
                        end
                    end)
                    WindUI:Notify{Title="锁定体力 --雪天缤纷", Content="已锁定 "..path.." 满值 "..tostring(maxv).."\n(若钓鱼仍掉血请截图此提示)", Duration=6}
                end)
            end
        end
    end
...
-- 设备模式监控：键盘/鼠标模拟类功能全部关闭的瞬间，立即恢复手机触控模式。
-- 功能开启期间不发送触摸（避免"轻点屏幕乱走"），关闭后一次性恢复。
local keyFuncActive = false
task.spawn(function()
    while task.wait(0.15) do
        local anyKey = getgenv().NNVN_AutoSkill
            or getgenv().NNVN_AutoNamelessMinigame
            or getgenv().NNVN_AutoPhase2
            or getgenv().NNVN_AutoBoss
            or getgenv().NNVN_AutoTicket
            or getgenv().NNVN_AutoHeal
        if anyKey then
            keyFuncActive = true
        elseif keyFuncActive then
            keyFuncActive = false
            RestoreTouchMode()
        end
    end
...
task.spawn(function()
    while task.wait(2) do
        local ws = getgenv().NNVN_WalkSpeed or 16
        local char = LocalPlayer.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum and hum.WalkSpeed ~= ws then pcall(function() hum.WalkSpeed = ws end) end
        end
    end
end)
...
-- 精准拉拔河：白线锁判定区中心（服务器判定基于白线位置，锁中心=必中）
-- 每帧强制应用（游戏每帧重置也扛得住）+ 动态查找（结构名变了也找得到）
local function findGreenZone(bossBar)
    -- 1. 优先按名字找
    local hitbox = bossBar:FindFirstChild("Hitbox")
        or bossBar:FindFirstChild("HitZone") or bossBar:FindFirstChild("SafeZone")
        or bossBar:FindFirstChild("Target")
    if hitbox then return hitbox end
    -- 2. 名字兜底：任何名字含 hit/zone/area/target 的 Frame
    for _, v in ipairs(bossBar:GetDescendants()) do
        if v:IsA("Frame") and (v.Name:lower():find("hit") or v.Name:lower():find("zone")
            or v.Name:lower():find("area") or v.Name:lower():find("target")) then
            return v
        end
    end
    -- 3. 颜色兜底：找背景偏绿的最大 Frame（绿区）
    local best, bestArea = nil, 0
    for _, v in ipairs(bossBar:GetDescendants()) do
        if v:IsA("Frame") then
            local c = v.BackgroundColor3
            local isGreen = c.G > c.R * 1.2 and c.G > c.B * 1.1
            local area = v.AbsoluteSize.X * v.AbsoluteSize.Y
            if isGreen and area > bestArea then best, bestArea = v, area end
        end
    end
    return best
end
-- 白线(Bar)动态查找：优先名字，兜底找轨道里非绿、窄长的 Frame
local function findWhiteBar(bossBar)
    local bar = bossBar:FindFirstChild("Bar") or bossBar:FindFirstChild("Slider")
        or bossBar:FindFirstChild("Pointer") or bossBar:FindFirstChild("Needle")
    if bar then return bar end
    for _, v in ipairs(bossBar:GetDescendants()) do
        if v:IsA("Frame") then
            local c = v.BackgroundColor3
            local isGreen = c.G > c.R * 1.2 and c.G > c.B * 1.1
            if not isGreen and v.AbsoluteSize.X > 4 and v.AbsoluteSize.X < 40 and v.AbsoluteSize.Y > 20 then
                return v
            end
        end
    end
...
-- 保存绿框原始状态（防止UI错位/上飞）
local bossGodSaved = {}
local function saveGreenZone(hb)
    if bossGodSaved.hb ~= hb then
        bossGodSaved = { hb = hb, size = hb.Size, pos = hb.Position, color = hb.BackgroundColor3, transp = hb.BackgroundTransparency }
    end
end
local function restoreGreenZone()
    if bossGodSaved.hb and bossGodSaved.hb.Parent then
        pcall(function()
            bossGodSaved.hb.Size = bossGodSaved.size
            bossGodSaved.hb.Position = bossGodSaved.pos
            bossGodSaved.hb.BackgroundColor3 = bossGodSaved.color
            bossGodSaved.hb.BackgroundTransparency = bossGodSaved.transp
        end)
    end
    bossGodSaved = {}
end
-- 精准拉拔河：白线锁定判定区中心（不铺满绿区，但白线锁住=点击必中）
local function applyBossGodMode(bossBar)
    pcall(function()
        local bar    = bossBar:FindFirstChild("Bar")
        local hitbox = findGreenZone(bossBar)
        if hitbox and bar and bar.Parent == bossBar then
            local hbLeft = hitbox.AbsolutePosition.X
            local hbCenterX = hbLeft + hitbox.AbsoluteSize.X * 0.5
            local targetX = hbCenterX - bossBar.AbsolutePosition.X - bar.AbsoluteSize.X * 0.5
            targetX = math.clamp(targetX, 0, math.max(0, bossBar.AbsoluteSize.X - bar.AbsoluteSize.X))
            bar.Position = UDim2.new(0, targetX, bar.Position.Y.Scale, bar.Position.Y.Offset)
        end
    end)
end

-- 锁白线应用节流（已弃用：原版逻辑不锁白线，保留函数避免引用错误）
local bossGodApplyAt = 0
local function applyGodModeThrottled(bossBar)
    local now = os.clock()
    if now < bossGodApplyAt then return end
    bossGodApplyAt = now + 0.12
    applyBossGodMode(bossBar)
end

-- ============================================================
-- 👑 Boss 战引擎（原版逻辑 + 帧率无关 + 高频点击）
-- 原版判定：白线对齐判定区 ±margin 内才点击（服务器按白线位置判定）
-- 增强：RenderStepped → task.wait(0.005) 帧率无关；点击频率提高
-- ============================================================
local ultraClickAt = 0
local ultraMouseOn = false
...
function p2ClickLoop()
    ...
    while getgenv().NNVN_AutoPhase2 do
        -- 动态查找（游戏重建也不怕）
        if not bossBar or not bossBar.Parent then
            bossBar = fishing:FindFirstChild("BossFightBar")
            task.wait(0.1)
            continue
        end
        if bossBar.Visible then
            local now = os.clock()
            local hitbox = findGreenZone(bossBar)
            local bar = findWhiteBar(bossBar) or bossBar:FindFirstChild("Bar")

            if hitbox and hitbox.Visible and bar and bar.Parent == bossBar then
                local barCenterX = bar.AbsolutePosition.X + bar.AbsoluteSize.X * 0.5
                local hbLeft     = hitbox.AbsolutePosition.X
                local hbRight    = hbLeft + hitbox.AbsoluteSize.X
                -- 判定窗口：margin 4%（原版）+ 容忍 1px
                local margin     = math.max(5, math.floor(hitbox.AbsoluteSize.X * 0.04))

                -- 白线对齐判定区才点击（原版逻辑：服务器按白线位置判定）
                if barCenterX >= (hbLeft - margin) and barCenterX <= (hbRight + margin) then
                    -- 首次进入时确保输入模式（触摸设备不发鼠标移动，避免切桌面模式）
                    if not ultraMouseOn then
                        ultraMouseOn = true
                        task.wait(0.03)
                    end
                    -- 高频点击（0.008s，帧率无关；手机触摸/PC鼠标自适应，不切桌面模式）
                    if now - ultraClickAt >= 0.008 then
                        ultraClickAt = now
                        local cx, cy = getGuiCenter(hitbox)
                        pcall(function() if hitbox.Activate then hitbox:Activate() end end)
                        SmartClickAt(cx, cy)
                    end
                end
            end
        end
        task.wait(0.005) -- 帧率无关（原版 RenderStepped 换成高频独立循环）
    end
    ultraMouseOn = false
end

-- 小游戏自愈：自动抛竿卡住时（钓鱼UI可见但自动点击失效>3s）自动重新激活
local miniGameWatchAt = 0
task.spawn(function()
    while task.wait(3) do
        if not getgenv().NNVN_AutoPhase2 then continue end
        pcall(function()
            local fishing = MainGui and MainGui:FindFirstChild("Fishing")
            if not fishing or not fishing.Visible then return end
            local bossBar = fishing:FindFirstChild("BossFightBar")
            if not bossBar or not bossBar.Visible then return end
            -- 如果 p2ClickLoop 协程死了，自动重启
...
RunService.RenderStepped:Connect(function(dt)
    -- 旋转陀螺：角色疯狂转圈（记录基准朝向，抛竿不受旋转影响）
    if getgenv().NNVN_Spinner and LocalPlayer.Character then
        local hrp = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
            if not getgenv().NNVN_SpinnerBase then
                getgenv().NNVN_SpinnerBase = hrp.CFrame -- 开启瞬间记录基准朝向
            end
            local speed = getgenv().NNVN_SpinnerSpeed or 500
            hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(speed) * math.min(dt, 0.1), 0)
        end
    elseif getgenv().NNVN_SpinnerBase then
        getgenv().NNVN_SpinnerBase = nil
    end
...
-- NoClip（流畅度优化：不再每帧遍历全部部件，改为 0.5s 一次 + 重生即时应用）
...
-- 28. FINALIZE
-- ====================================================
-- 灵动感：主界面背景雪花粒子（轻量、不挡操作，呼应雪天主题）
task.spawn(function()
    task.wait(2.5) -- 等启动动画结束后再出现
    pcall(function()
        ...
        for i = 1, 6 do
            ...
            p.Parent = sg
            table.insert(parts, {label = p, speed = math.random(3, 8) / 100, drift = (math.random() - 0.5) * 0.5, rot = math.random(-40, 40)})
        end
        while true do
            task.wait(0.2)
            for _, pt in ipairs(parts) do
                ...
                local ny = pos.Y.Scale + pt.speed / 60
                local nx = pos.X.Scale + pt.drift / 60
                p.Rotation = p.Rotation + pt.rot / 60
                if ny > 1.06 then
                    p.Position = UDim2.fromScale(math.random(), -0.06)
                    p.Rotation = 0
                else
...
-- UI 文字高亮：背景图较暗，统一提亮文字 + 深色描边保证清晰
task.defer(function()
    pcall(function()
        for _, child in ipairs(Window:GetDescendants()) do
            if child:IsA("TextLabel") or child:IsA("TextButton") then
                child.TextColor3 = Color3.fromRGB(255, 246, 255)
                child.TextStrokeTransparency = 0.25
                child.TextStrokeColor3 = Color3.fromRGB(15, 8, 35)
            end
        end
    end)
end)
...
if getgenv().NNVN_Notify then
    WindUI:Notify{
        Title    = "雪天中心 v1.2 Beta [WindUI] --雪天缤纷",
        Content  = T.loaded.."\ndiscord.gg/5JJAuHRUgJ\nWindUI loaded",
        Duration = 6
    }
    -- 📢 v1.2 Beta 启动公告（等启动动画完成后显示，确认后进入）
    task.spawn(function()
        -- 等待启动动画完成（或跳过/超时）
        for _ = 1, 80 do
            if getgenv().NNVN_BootDone then break end
            task.wait(0.25)
        end
        task.wait(0.6)
        pcall(function()
            local sg = Instance.new("ScreenGui")
            sg.Name = "NNVN_Announce"
            sg.ResetOnSpawn = false
            sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            sg.DisplayOrder = 200 -- 最高层，保证可点击不被遮挡
            ...
            card.Size = UDim2.fromOffset(440, 330)
            ...
            title.Font = Enum.Font.NotoSans; title.FontWeight = Enum.FontWeight.Bold
            title.TextSize = 22
            title.Text = "雪天中心 v1.2 Beta"
            ...
            sub.Font = Enum.Font.NotoSans
            sub.TextSize = 12
            sub.Text = "更新公告 | 作者：雪天缤纷Pro"
            sub.TextColor3 = Color3.fromRGB(180, 170, 220)
            sub.Parent = card

            local content = Instance.new("TextLabel")
            content.Position = UDim2.fromScale(0.06, 0.24)
            content.Size = UDim2.fromScale(0.88, 0.58)
            content.BackgroundTransparency = 1
            content.Font = Enum.Font.NotoSans
            content.TextSize = 13.5
            content.TextXAlignment = Enum.TextXAlignment.Left
            content.TextYAlignment = Enum.TextYAlignment.Top
            content.TextWrapped = true
            content.Text = "v1.2 Beta 更新内容：\n\n✦ 启动修复：UI 三级加载（缓存/远程/内嵌克隆库），不挂梯子也能启动\n✦ ⚡ 挂机中心：一键挂机循环 + 收益统计 + 智能钓点 + Boss循环 + 自动拾取\n✦ 🌐 任务翻译：查看所有任务并翻译成中文\n✦ 🎯 精准拉拔河：Enzo/章鱼 拉拔河必中（白线锁判定区）\n✦ 🛠 传送修复：岛屿传送/天气岛屿 全部恢复\n✦ ✨ UI 灵动动画 + 全中文 NotoSans 字体\n✦ 💗 自动回血 + 🧠 智能卖鱼（保留鱼种）"
            ...
            btn.Font = Enum.Font.NotoSans; btn.FontWeight = Enum.FontWeight.Bold
            btn.TextSize = 16
            btn.Text = "确认开始"
            ...
            local function onConfirm()
                sg:Destroy()
                if getgenv().NNVN_Notify then
                    WindUI:Notify{Title="雪天中心 --雪天缤纷", Content="v1.2 Beta 已就绪", Duration=3}
                end
            end
            ...
            -- 手动确认进入（防卡死 60 秒超时兜底）
            task.delay(60, onConfirm)
        end)
    end)
end

-- ============================================================
-- 🩺 功能健康自检：运行后输出各系统状态（定位问题用）
-- ============================================================
task.spawn(function()
    task.wait(4)
    pcall(function()
        local okList, failList = {}, {}
        local function check(name, cond)
            if cond then table.insert(okList, name) else table.insert(failList, name) end
        end
        check("UI窗口", type(Window) == "table")
        check("自动抛竿", type(getgenv().NNVN_AutoCast) == "boolean")
        check("至尊Enzo", type(p2ClickLoop) == "function")
        check("章鱼引擎", type(handleNamelessRhythmMinigame) == "function")
        check("任务系统", type(getgenv().NNVN_GetQuestProgress) == "function")
        check("传送修复", type(matTeleportToIsland) == "function")
        check("挂机中心", type(getgenv().NNVN_AutoAFK) == "boolean")
        check("收益统计", type(FindMoneyValue) == "function")
        check("智能卖鱼", type(getgenv().NNVN_SmartSell) == "boolean")
        check("自动回血", type(getgenv().NNVN_AutoHeal) == "boolean")
        check("任务翻译", type(TranslateQuestDesc) == "function")
        check("公告", game.Players.LocalPlayer.PlayerGui:FindFirstChild("NNVN_Announce") ~= nil)
        print("[雪天中心] 🩺 健康自检: 正常 " .. #okList .. " | 异常 " .. #failList)
        for _, n in ipairs(failList) do print("   ❌ " .. n) end
        if #failList > 0 and getgenv().NNVN_Notify then
            WindUI:Notify{Title="雪天中心 --雪天缤纷", Content="⚠ 有 "..#failList.." 个模块未就绪: "..table.concat(failList, ", "), Duration=6}
        end
    end)
end)
    ...
    LocalPlayer.OnTeleport:Connect(function()
        if not isTeleporting then
            ...
            queueteleport([[
                ...
                pcall(function()
                    local src
                    for _, f in ipairs({"xuetianzhongxin.lua", "main_script_cn.lua", "xuetianzhongxin.txt"}) do
                        local ok, d = pcall(readfile, f)
                        if ok and d and #d > 100000 then src = d; break end
                    end
                    if src then loadstring(src)(); okLoad = true end
                end)
                if not okLoad then
